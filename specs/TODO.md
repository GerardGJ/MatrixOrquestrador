...
- [x] Generate a PDF once the full DAG execution is complete.
- [x] Include an execution summary section in the PDF.
...
- [ ] The PDF table should contain at least:
  ...
  - [x] Total execution time.
  - [ ] Output rows.
- [x] Define the output location and naming convention for generated PDF reports.
...
- [ ] Separate responsibilities into components:
  - [ ] DAG builder/parser.
  - [x] DAG validator.
  - [x] Execution scheduler.
  - [ ] Process runner.
  - [ ] Metrics/logger service.
  - [x] PDF report generator.
- [x] Keep process implementations decoupled from orchestration infrastructure.
- [x] Make it easy to plug in dummy steps now and real database/JDBC steps later.
...
- [x] Implement PDF export from the aggregated execution logs.
...
- [x] A process never starts before all its dependencies are completed.
- [x] Independent DAG branches can execute in parallel.
- [x] Every executed process produces a structured execution log.
- [ ] The final execution produces a PDF report with all process logs in a table.
- [x] The orchestrator is easy to extend with new process nodes and dependency changes.
